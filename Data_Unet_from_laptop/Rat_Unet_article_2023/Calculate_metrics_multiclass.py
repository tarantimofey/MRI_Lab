...
path = '/media/taran/SSD2/Data_Unet_from_laptop/Rat_Unet_article_2023/Trained_models/unet3plus_deepmeta/Torch-deepmeta'
Y_folder = path + '/outputs/'
Y_ground_folder = path + '/labels/'
...
for i in range(len(Y)):
    Y_img = np.zeros((128,128),np.uint8)
    Y_img_ground = np.zeros((128,128),np.uint8)
    Y_img[Y[i]==class_n] = 1
    Y_img_ground[Y_ground[i]==class_n] = 1
